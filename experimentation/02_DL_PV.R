...
dir.create("Results/PV/models", recursive = T, showWarnings = FALSE)
...
for (W in seq(from = 20, to = 140, by = 20)){
  ...
  predictors=c(1:W)
  train = read.csv(file = paste0("data/training/2015_PV_W",W,"_H",H,".csv"), header = TRUE, sep = ";", dec = ",")
  test = read.csv(file = paste0("data/test/2016_PV_W",W,"_H",H,".csv"), header = TRUE, sep = ";", dec = ",")
  ...
  for(h in 1:H){
    ...
    h2o.saveModel(object=model, path=paste0("Results/PV/models/model_W",W,"_H",h), force=TRUE)
    sink(paste0("Results/PV/models/DetailsModel_W",W,"_H",h,".txt"))
    print(summary(model))
    ...
    write.csv(realVsPred, paste0("Results/PV/W",W,"_H",h,".csv"), col.names = TRUE, row.names = FALSE)
    print(paste0("\t## Fin prediccion Sub-",h))
  } #Termina el entrenamiento de todos los subproblemas
  
  sink(paste0("Results/PV/models/timesFor_W",W,"_H",h,".txt"))
