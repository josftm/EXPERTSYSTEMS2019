...
dir.create("Results/PV_Forecast/models",recursive = T, showWarnings = F)
...
for (W in seq(from = 20, to = 140, by = 20)){
  for(ruido in c(1:3)){
    print(paste0("## Inicio entrenamiento PV_W",W, " con ruido ",ruido))
    ...
    test = cbind(PVTest, FTest)
    test = test[,c(1:W,(W+H+1):ncol(test),(W+1):(W+H))]
    
    predictors=c(1:(W+(4*W/20)))
    ...
    for(h in 1:H){
      ...
      h2o.saveModel(object=model, path=paste0("Results/PV_Forecast/models/model_W",W,"_H",h,"_Ruido",ruido), force=TRUE)
      sink(paste0("Results/PV_Forecast/models/DetailsModel_W",W,"_H",h,"_Ruido",ruido,".txt"))
      print(summary(model))
      ...
      write.csv(realVsPred, paste0("Results/PV_Forecast/W",W,"_H",h,"_Ruido",ruido,".csv"), col.names = TRUE, row.names = FALSE)
      print(paste0("\t## Fin prediccion Sub-",h))
    } #Termina el entrenamiento de todos los subproblemas
    
    sink(paste0("Results/PV_Forecast/models/timesFor_W",W,"_H",h,"_Ruido",ruido,".txt"))
