...
dir.create("Results/PV_ForecastWeather/models",recursive = T, showWarnings = F)
...
for (W in seq(from = 20, to = 140, by = 20)){
  ...
  trainAux = cbind(PVTrain, WTrain)
  
  WTest = read.csv(file = paste0("data/test/2016_Weather_W",(W/20),".csv"), header = TRUE, sep = ",", dec = ".")
  WTest = WTest[1:nrow(PVTest),]
  testAux = cbind(PVTest, WTest)
  
  for(ruido in c(1:3)){
    FTrain = read.csv(file = paste0("data/training/2015_ForWeather_W",(W/20),"_Noise",ruido,".csv"), header = TRUE, sep = ",", dec = ".")
    FTrain = FTrain[1:nrow(PVTrain),]
    train = cbind(trainAux, FTrain)
    train = train[,c(1:W,(W+H+1):ncol(train),(W+1):(W+H))]
    
    FTest = read.csv(file = paste0("data/test/2016_ForWeather_W",(W/20),"_Noise",ruido,".csv"), header = TRUE, sep = ",", dec = ".")
    FTest = FTest[1:nrow(PVTest),]
    test = cbind(testAux, FTest)
    test = test[,c(1:W,(W+H+1):ncol(test),(W+1):(W+H))]
    
    predictors=c(1:(ncol(train)-20))
    ...
    for(h in 1:H){
      ...
      h2o.saveModel(object=model, path=paste0("Results/PV_ForecastWeather/models/model_W",W,"_H",h,"_Ruido",ruido), force=TRUE)
      sink(paste0("Results/PV_ForecastWeather/models/DetailsModel_W",W,"_H",h,"_Ruido",ruido,".txt"))
      print(summary(model))
      ...
      write.csv(realVsPred, paste0("Results/PV_ForecastWeather/W",W,"_H",h,"_Ruido",ruido,".csv"), col.names = TRUE, row.names = FALSE)
      print(paste0("\t## Fin prediccion Sub-",h))
    } #Termina el entrenamiento de todos los subproblemas
    
    sink(paste0("Results/PV_ForecastWeather/models/timesFor_W",W,"_H",h,"_Ruido",ruido,".txt"))
